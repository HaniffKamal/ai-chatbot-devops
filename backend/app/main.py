...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.error("Failed to initialize RAG pipeline: %s", exc, exc_info=True)
...
async def chat_endpoint(request: ChatRequest):
    ...
    if rag_pipeline:
        try:
            retrieved_context = rag_pipeline.retrieve_context(request.message, n_results=2)
            system_prompt = rag_pipeline.build_system_prompt(retrieved_context)
            logger.info("RAG context retrieved (%d chars) for query: '%s'", len(retrieved_context), request.message[:60])
        except Exception as exc:
